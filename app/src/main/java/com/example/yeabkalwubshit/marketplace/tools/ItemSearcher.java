...
/**
 * Implements item searching from database by title and category.
 */
public class ItemSearcher {
    ...
    public ArrayList<Item> runQuery(String queryString, int maxResult) {
        ...
        boolean isCategorySeachQuery = false;
        if(queryString.startsWith("#")) {
            isCategorySeachQuery = true;
        ...
        for(String itemId: items.keySet()) {
            int corelation = getLongestCommonSubsequenceLength(
                    (isCategorySeachQuery ? items.get(itemId).getCategory().toLowerCase()
